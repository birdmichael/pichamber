  ...
  async stopRemoteServerBestEffort(parsed, controlPath, remotePort) {
    try {
      await this.runRemoteCommand(
        parsed,
        controlPath,
        `pichamber stop --port ${remotePort} >/dev/null 2>&1 || true; if command -v curl >/dev/null 2>&1; then curl -fsS -X POST http://127.0.0.1:${remotePort}/api/system/shutdown >/dev/null 2>&1 || true; elif command -v wget >/dev/null 2>&1; then wget -qO- --method=POST http://127.0.0.1:${remotePort}/api/system/shutdown >/dev/null 2>&1 || true; fi`,
      );
    } catch {
    }
  }
