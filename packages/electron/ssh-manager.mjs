...
const REMOTE_BIN_CANDIDATES = [
  '"$HOME/.pichamber/npm-global/bin/pichamber"',
  '"${BUN_INSTALL:-$HOME/.bun}/bin/pichamber"',
  '"$HOME/.local/bin/pichamber"',
  '"$HOME/.openchamber/npm-global/bin/pichamber"',
...
export class ElectronSshManager {
  ...
  // Every place Pichamber (or leftover OpenChamber) may live on the remote host,
  // with the version each one reports. Installs land in the user prefix while an
  // older copy can still sit on PATH, so the caller picks by version instead of
  // trusting PATH order. Do not require the leftover OpenCode CLI.
  ...
  async installOpenChamberManaged(parsed, controlPath, version, preferred) {
    ...
    const bunCommand = bunPath ? `${shellQuote(bunPath)} add -g @pichamber/web@${version}` : null;
    const npmCommand = npmPath
      ? `mkdir -p "${REMOTE_USER_PREFIX}" && ${shellQuote(npmPath)} install -g --prefix "${REMOTE_USER_PREFIX}" @pichamber/web@${version}`
      : null;
  ...
  async ensureRemoteServer(instance, parsed, controlPath) {
    ...
    if (!binary) {
      ...
      if (existing) {
        this.setStatus(instance.id, 'updating', `Updating remote Pichamber from ${existing.version || 'unknown'} to ${this.appVersion}`);
      } else {
