...
Desktop waits for `/health` `kernelReady` / `piRunning` after the in-process server is listening. A still-starting Node kernel child (`PI_SDK_UNAVAILABLE`, `ok: false`) is not a hard boot failure. Retry Local re-probes live health and only restarts when the local HTTP origin is not active. Chooser/local-setup on the Pi kernel show Pi install/docs and `piBinaryResolved` from that health payload. Startup-failed / Local Pi unavailable recovery shows the same Pi install/docs. They do not search for or spawn `opencode`. Desktop still does not spawn PATH `pi`.
