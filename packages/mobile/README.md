...
- The native app bundles the mobile UI only; it does not embed the Pichamber web server or OpenCode server.
...
- Phones and tablets share one navigation model: a sessions drawer/sidebar on the left, the workspace drawer (Git / Files / Terminal / Notes / Plan / MCP) on the right, and no overflow menu. Plan is the Desktop View Plan surface and appears only when Feature Plugins Plan is installed and enabled and the session plan is on (including an empty “no plan yet” state). The header context ring opens Work Status via `MobileWorkStatusHost` — the same session, branch, context %, MCP, and Subagents rows as Desktop. The 300px chat-column card stays off. MCP and Subagents follow those Feature Plugin slots; tapping a live child opens that chat in place. Usage quotas stay hidden on Pi except the explicitly enabled Grok, Kimi Code, and Z.AI Usage Feature Plugin surfaces. Z.AI mirrors Desktop Work Status: domestic quota windows are shown, while international usage is unavailable rather than shown as zero. From Git, Pull Request opens the same Desktop view as the macOS rail (`PullRequestView`). File diffs stay inline on phones (`MobileDiffDetail`) and use Desktop `DiffView` on tablet-width screens (768px and up). Walkthrough is tablet-only from those views; phones do not get a second stacked Walkthrough. Browser is unsupported — the mobile WebView cannot offer the Electron Chromium session. The sessions sheet also reaches Desktop session chrome: Scheduled Tasks, Multi-run, Archive with restore, and Pi Refresh (same Pi APIs and `sessions.sidebar.*` wording). Tablets differ only in that the sessions list is a resizable persistent sidebar and the header dropdowns are anchored popovers.
