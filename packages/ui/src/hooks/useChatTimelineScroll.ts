...
export const useChatTimelineScroll = ({
    ...
    // Capacitor keyboard slide: the shell moves the composer with a transform
    // and then snaps layout. Pinning against those intermediate sizes fights
    // the choreography, so follow writes stand down until `oc:keyboard-settled`.
    ...
    React.useEffect(() => {
        ...
        const observer = new ResizeObserver((observerEntries) => {
            ...
            quietTimer = setTimeout(() => {
                ...
                // Pinned readers (idle or streaming) stay on the measured end
                // through panel/window resizes; unpinned readers are held by
                // list size compensation instead.
    ...
    React.useEffect(() => {
        ...
        const pin = () => {
            ...
            if (widthResizingRef.current || keyboardAnimRef.current) return;
