...
| `session-ui-store.ts` | Session selection, draft lifecycle (including local draft Plan intent, last empty-composer Plan, and chat-vs-project draft target), abort prompts, worktree metadata, SDK-facing action entrypoints | App UI state |
