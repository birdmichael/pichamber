...
| `pi-session-plan-store.ts` | Live Pi plan-mode status (`off` / `active` / `ready` / `saved` / `implementing`) from `GET /api/pi/session/:id/plan` | One runtime; keyed by Pi session ID. Fetch failure does not become empty `off`. A GET that still says `off` must not overwrite optimistic or just-started `active` / `ready`. Applying `ready` with markdown, or a pending plan-ready `ctx.ui.select`, opens the Desktop Plan rail via `openContextPlan` (`expanded: false`, never desktop `activeMainTab=plan`). Applying `active` from GET/event does not. A live composer `/plan start` still opens that rail. Draft Plan is toast + composer row only. A new-session draft stores Plan as `newSessionDraft.planSelected`. Last empty-composer Plan (`emptyComposerPlanSelected`) survives sidebar session switches and is restored when that draft reopens. A later New session starts on Agent and does not inherit the previous draft’s Plan. Send or an explicit Agent pick consumes it. `/plan start` still waits until send materializes that session. The Desktop Agent/Plan chip keeps draft Plan through first-send materialize. `usePiPlanChrome` resolves the open chat from `currentSessionId`, then URL `?session=`, then last-active, so an auto-draft welcome cannot turn Agent→Plan into draft intent or hide GET status for that id. Host GET/POST also read the session jsonl file when the in-memory snapshot omits `plan-mode-state`. |
...
4a. A new-session draft send paints a pending composer turn immediately (`pending-composer-turn.ts`, not a placeholder session id). After `POST /session` returns the Pi UUID, `optimisticAdd` owns the bubble and the pending turn is cleared. Goal/plan/response-style work runs in `beforeSend` after that insert. A Plan-selected draft applies `active` before `/plan start` so a stale `GET /plan` `off` cannot revert the chip; start failure writes `off`. Create/send failure restores the composer and clears the pending turn. Authoritative messages for that session also clear any leftover pending turn.
