...
| `pi-feature-plugins-store.ts` | Feature Plugins payload for composer/rail gates (`plan` and `goal` installed+enabled) | One runtime. Fetch failure does not become an empty disabled snapshot |
