...
| `pi-session-plan-store.ts` | Live Pi plan-mode status (`off` / `active` / `ready` / `saved` / `implementing`) from `GET /api/pi/session/:id/plan` | One runtime; keyed by Pi session ID. Fetch failure does not become empty `off`. Applying `ready` with markdown, or a pending plan-ready `ctx.ui.select`, opens the Desktop Plan rail via `openContextPlan` (`expanded: false`, never desktop `activeMainTab=plan`). A live composer `/plan start` also opens that rail. Draft Plan is toast + composer row only. A new-session draft stores Plan as `newSessionDraft.planSelected`. A later New session starts on Agent and does not inherit the previous draft’s Plan. Send or an explicit Agent pick still consumes this draft’s Plan. `/plan start` still waits until send materializes that session. `usePiPlanChrome` resolves the open chat from `currentSessionId`, then URL `?session=`, then last-active, so an auto-draft welcome cannot turn Agent→Plan into draft intent or hide GET status for that id. Host GET/POST also read the session jsonl file when the in-memory snapshot omits `plan-mode-state`. |
