...
| `session-ui-store.ts` | Session selection, draft lifecycle (including local draft Plan intent and chat-vs-project draft target), abort prompts, worktree metadata, SDK-facing action entrypoints. A user-initiated `openNewSessionDraft` (not `automatic`, no `initialPrompt`) clears the new-session composer draft so a leftover `/` cannot reopen the slash menu. `routeMessage` uses `parseSlashCommand`: a sentence that starts with `Plan ` (no slash) is `sendMessage`, not `/plan` `session.command`. | App UI state |
| `pi-extension-ui-store.ts` | Pending/settled Pi `ctx.ui` prompts (`select` / `confirm` / `input` / `editor`) and `pi.ui.notify` toasts for the current runtime | One runtime; prompts keyed by Pi session ID. Not OpenCode `question`. Opening a session hydrates `GET /api/pi/ui`; fetch failure does not clear local cards. `focusPromptId` is a one-shot scroll/highlight from a notification deep link (`?prompt=`). Pending `select` / `input` / `editor` cards that match a visible **active** (`pending` / `running`) `question` / `plan_mode_question` tool part render on that turn. Dock binding uses the rendered message snapshot, not live `state.part` ahead of MessageList — otherwise a pending select is hidden with no card and blocks send. A completed or cancelled question tool must not steal a later pending prompt (the plan-ready `Proposed plan ready. What next?` select stays in the dock). The bottom dock keeps only the latest unbound pending prompt (one live ask). After reply or dismiss, the answered or cancelled result stays on the asking tool part — not as a sticky card under later messages. A pending blocking prompt counts toward sidebar attention so the chat stays findable after you leave. Select cards treat `Other` and Pi `Type something.` (including numbered variants) as freeform so the in-chat textarea opens; Plan Other still stashes text for the next `editor`. Notify toasts present on the shared Sonner surface when the event is applied, not only from a later React effect. Routine `Session backfill complete` plugin status does not toast or enter the notify queue; `Session backfill failed` / `check failed` still do. Info and warning notifies are short auto-dismiss toasts at `top-center` without an OK action so they do not cover the composer. Errors stay longer and keep Copy. |
| `pi-session-plan-store.ts` | Live Pi plan-mode status (`off` / `active` / `ready` / `saved` / `implementing`) from `GET /api/pi/session/:id/plan` | One runtime; keyed by Pi session ID. Fetch failure does not become empty `off`. A GET or `pi.plan.updated` that still says `off` must not overwrite optimistic or just-started `active` / `ready`, or a pending first-send Plan. Applying `ready` with markdown, or a pending plan-ready `ctx.ui.select`, opens the Desktop Plan rail via `openContextPlan` (`expanded: false`, never desktop `activeMainTab=plan`). Applying `active` from GET/event does not. A live composer `/plan start` still opens that rail. A plan-ready `pi.ui.asked` also `GET`s the session plan so an already-open empty rail is not stuck waiting for remount. `pi.plan.updated` from `plan_mode_complete` is the live markdown path. View Plan Build/Save/Discard answers that pending select (`Implement here` / `Save for later` / `Discard plan and exit`) instead of sending a second `/plan implement` while the card is still open. Draft Plan is toast + composer row only. A new-session draft stores Plan as `newSessionDraft.planSelected`. Last empty-composer Plan (`emptyComposerPlanSelected`) survives sidebar session switches and is restored when that draft reopens. A later New session starts on Agent and does not inherit the previous draft’s Plan. Send or an explicit Agent pick consumes it. `createSession` adopts that draft Plan onto the new session (`active` + pending) before it closes the draft, so the Desktop chip does not flip to Agent. `/plan start` still waits until send materializes that session. A failed start must not write `off`. `usePiPlanChrome` resolves the open chat from `currentSessionId`, then URL `?session=`, then last-active, so an auto-draft welcome cannot turn Agent→Plan into draft intent or hide GET status for that id. Host GET/POST also read the session jsonl file when the in-memory snapshot omits `plan-mode-state`. After user-initiated implement, `session.idle` (`agent_settled`) settles leftover `implementing` to `off` without clearing `implemented`, so a later GET ready/implementing cannot restore **building…**. Live building chrome is `status === implementing` and a busy/retry turn — leftover adapter busy is not live. |
| `pi-feature-plugins-store.ts` | Feature Plugins payload for composer/rail gates (`plan` and `goal` installed+enabled) and the typed `/run` `/plan` `/goal` late-list `session.command` bypass | One runtime. Fetch failure does not become an empty disabled snapshot. A missing or inactive slot sends those slashes as chat; an active slot still uses `session.command` when `GET /api/command` has not listed them yet. Enable of an installed Goal slot reloads idle sessions; the composer Goal button may mint a draft session before `session.command` only when `currentSessionId`, the URL `?session=`, and last-active session are all empty. An open chat must keep `/goal` on that session. |
| `useGlobalSessionsStore.ts` | Global active sessions, global archived sessions, `sessionsByDirectory` | All opened project/worktree session lists, plus a runtime-scoped managed-chats snapshot for web/desktop cold start |
...
The active-session watchdog in `sync-context.tsx` (per-directory status polls and child-session discovery lists) runs its network calls through the shared background-network gate in `@/lib/background-network`, alongside poll-shaped git reads, global session pages, and command/skill discovery. Background fan-out must stay under that gate so the browser's per-origin connection pool keeps free sockets for interactive traffic — an uncapped startup burst previously queued the first session-open message fetch for seconds. The interval stays armed while the window is hidden (`document.hidden`, same gate as FilesView), but ticks do no network work. Becoming visible again clears the per-directory poll timestamps and runs one refresh immediately.

Desktop tray (`useTraySync`) is event-driven. The 5s status poll and 45s `refreshGlobalSessions` safety nets stay off when the menu bar is disabled or when no tray session is `busy`/`retry`. A later busy/idle event still flushes a snapshot immediately. Hidden-document ticks do no network work; becoming visible still refreshes once.
...
9. Transcript arrays are chronological by `message.time.created`, with message ID used only as a deterministic equal-time tie-breaker. Message IDs are identity and reconciliation keys, not chronology: OpenCode's fixed-width sortable timestamp prefix rolls over, so a newer `msg_000...` can follow an older `msg_fff...`. Fetch, pagination, materialization, optimistic insertion, events, reconnect inspection, rendering, and revert/undo/redo must preserve this contract. One send is one user turn: a client `msg_*` / `usr_*` and a Pi jsonl id with the same text are the same turn. Confirm the optimistic item, keep the client id, and drop the hydrate echo — including when the optimistic shadow is already empty and when `message.updated` arrives with the Pi id. Two client ids or two Pi ids with the same text stay (a resend). A Pi `/goal` user bubble and its Goal-turn assistants are restamped so `/goal` sorts before those replies after jsonl hydrate.
10. Part arrays preserve authoritative response/event order. Part IDs are identity keys and have the same rollover limitation; identity lookup/removal must not require a part array to be lexically ID-sorted. A server `message.part.updated` for a just-sent user message replaces the matching optimistic text/file part in place. Appending it would reorder those parts and remount the rendered subtree.
...
4a. A new-session draft send paints a pending composer turn immediately (`pending-composer-turn.ts`, not a placeholder session id). After `POST /session` returns the Pi UUID, `optimisticAdd` owns the bubble and the pending turn is cleared. Goal/plan/response-style work runs in `beforeSend` after that insert. A Plan-selected draft applies `active` before `/plan start` so a stale `GET /plan` `off` cannot revert the chip; start failure must not write `off`. Create/send failure restores the composer and clears the pending turn. Authoritative messages for that session also clear any leftover pending turn.
...
8. Temporary `/btw` forks are marked `openchamber.kind = 'btw'` before they enter a directory store. Sidebar, switcher, and command-palette lists omit those sessions until Keep strips the marker. Deleting or archiving a parent also deletes its linked btw fork; deleting a fork unlinks the parent.
