...
4a. A new-session draft send paints a pending composer turn immediately (`pending-composer-turn.ts`, not a placeholder session id). After `POST /session` returns the Pi UUID, `optimisticAdd` owns the bubble and the pending turn is cleared. Goal/plan/response-style work runs in `beforeSend` after that insert. Create/send failure restores the composer and clears the pending turn.
