...
`useXaiUsageStore` caches `GET /api/pi/xai-usage` for Work Status Usage and the Providers xAI card, keyed by provider id (`byId.xai`, `byId.xai-2`, …). A failed fetch or configured `{ ok: false }` keeps that id’s last successful `ok` snapshot and must not become empty 0% usage. First-load failure stays an error, not "not signed in". A refresh while that id’s request is in flight is queued so the click is not dropped. Fetching `xai-2` must not replace `xai`. `reset()` invalidates in-flight results (runtime switch). Disconnected xAI providers hide both surfaces; package installation is not consulted.

`useKimiUsageStore` is the parallel cache for `GET /api/pi/kimi-usage` (Work Status Usage and the Providers Kimi Code card), also keyed by provider id. Same reconcile rules per id. Work Status lists one row per connected Grok/Kimi Code subscription (display name from the usage payload). Disconnected Kimi providers hide both surfaces; only Kimi Code OAuth credentials count; dual-auth `kimi-coding-api` and Completions API-key siblings (including numeric clones named like "Kimi API") are excluded from discovery, fetch, and re-login CTA. Pi Work Status Usage follows connected xAI/Kimi providers in stable family order.
...
- The OpenCode `archived` list flag means "also include archived sessions": the leftover kernel only drops its `time_archived IS NULL` condition. On OpenCode the global cache therefore loads with one inclusive request (`archived: true`) and splits active/archived client-side via `splitGlobalSessionsByArchived` — an `archived: false` request cannot be truthful because that filter excludes restored sessions (`time.archived` falsy-but-present, see "Restore (unarchive) contract" in `sync/DOCUMENTATION.md`). Pi writes `archived: 0` and daily startup / sidebar / tray / cleanup loads use `archived: false`. Archive page and the VS Code archived bucket call `loadArchivedSessions()` (`archived: true`) when that surface opens. An active-only Pi load preserves an already-fetched archived bucket except for IDs that reappear as active (restored). For callers that still want only archived records, `listGlobalSessionPages` narrows inclusive responses at the data boundary (default `narrowToArchived`), so the archived cache never holds active sessions and no consumer has to re-derive that. Pagination progress stays measured on the raw response, so a page that is full upstream but filtered out here is not mistaken for the last page.
- Per-directory refresh issues one request per directory (previously two), bounded to two requests across callers and prioritizing the current directory. OpenCode keeps that request inclusive; Pi uses the same active-only query as the global load.
...
Project ordering defaults to manual. Session display persistence v3 migrates the previously shipped `recent` project order to `manual` while preserving every other explicit sort mode. v5 adds an independent all-projects / one-project sidebar view; the selected project id stays device-local.
