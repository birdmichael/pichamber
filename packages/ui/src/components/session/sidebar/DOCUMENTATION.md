...
- Layout (web/desktop): top navigation (`SidebarNav`: New session, Scheduled, Multi-run, Archive), then an optional `recent` fallback zone for sessions that have no workspace/project row, then one zone per project with a **flat** session list. There is no rendered worktree grouping level. A session is never listed in Recent and a workspace group at the same time.
