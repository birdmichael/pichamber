...
- `SidebarHeader.tsx`: Top header UI for add-project, session search, selection mode, project sort, and the display menu (recent toggle, collapse/expand all). The Scheduled Tasks calendar button is a Pichamber scheduler entry and stays visible on Pi. New multi-run stays visible on Pi and opens the existing launcher.
...
- `activitySections.ts`: Recent membership helpers. `deriveRecentSessions` is the timestamp/active filter; `selectRecentSessionsWithoutWorkspaceGroup` drops sessions that already have a workspace/project row so Recent cannot duplicate them.
