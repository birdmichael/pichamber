...
- `SidebarHeader.tsx`: Top header UI for add-project, session search, selection mode, project sort, and the display menu (recent toggle, collapse/expand all). The Scheduled Tasks calendar button is OpenCode-only and hidden when `usePiKernel()` is true.
