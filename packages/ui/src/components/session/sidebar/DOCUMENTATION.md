...
- `SidebarHeader.tsx`: Top header UI for add-project, session search, selection mode, project sort, and the display menu (recent toggle, collapse/expand all). The Scheduled Tasks calendar button is a Pichamber scheduler entry and stays visible on Pi. New multi-run merge-arrows stay hidden when `usePiKernel()` is true.
