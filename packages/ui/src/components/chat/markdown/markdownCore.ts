...
const streamBlocks = (text: string, live: boolean): MarkdownBlock[] => {
  ...
  try {
    // Use the same parser that owns disclosure/math extensions so unfinished
    // <details> bodies stay one streamed block instead of splitting on blanks.
    tokens = inlineImageParser.lexer(text) as Tokens.Generic[];
  } catch {
    return [{ raw: text, src: heal(text), mode: 'live', highlight: true }];
  }
