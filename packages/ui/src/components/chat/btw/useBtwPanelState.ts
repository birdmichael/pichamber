...
export function useBtwPanelState(
  ...
  // The fork may live in a server-canonicalized directory store, so look it
  // up in the parent directory first and then across live stores.
  const btwSessionInDirectory = useSession(linkedBtwSessionId, directory);
  const btwSessionAnywhere = useSession(linkedBtwSessionId);
  const btwSession = btwSessionInDirectory ?? btwSessionAnywhere ?? null;
