...
export const SubagentRosterMenu: React.FC<Props> = (props) => {
  ...
  return <div className='w-[min(420px,calc(100vw-1rem))] p-2 text-xs' onKeyDown={(event) => event.stopPropagation()}>
    ...
    <div className='mt-2 border-t border-border/50 pt-2'><div className='flex items-center justify-between px-2 pb-1 font-semibold text-muted-foreground'><span>{t('chat.agentRoster.subagents')}</span><div className='flex items-center gap-1'><button type='button' className='rounded px-1.5 py-0.5 text-muted-foreground hover:bg-muted/60' onClick={props.onManage}>{t('chat.agentRoster.manage')}</button><button type='button' className='rounded px-1.5 py-0.5 text-primary hover:bg-primary/10' onClick={() => { selectAgent(null); setEditorOpen(true); }}>{t('chat.agentRoster.new')}</button></div></div>
