...
import {
  isShortcutRecordingDismissEvent,
  settleShortcutRecordingState,
  updateShortcutRecordingState,
} from './ShortcutRecordingDialog';
...
describe('ShortcutRecordingDialog recording state', () => {
  ...
  test('records Enter while Backspace removes the final chord', () => {
  ...
  test('Escape and X close the dialog instead of recording a chord', () => {
    expect(isShortcutRecordingDismissEvent({ key: 'Escape' })).toBe(true);
    expect(isShortcutRecordingDismissEvent({ key: 'Enter' })).toBe(false);
    expect(isShortcutRecordingDismissEvent({ key: 'x' })).toBe(false);
  });
});
