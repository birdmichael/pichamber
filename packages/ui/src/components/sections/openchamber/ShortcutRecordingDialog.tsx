...
// eslint-disable-next-line react-refresh/only-export-components -- tested dialog dismiss contract
export function isShortcutRecordingDismissEvent(event: Pick<KeyboardEvent | React.KeyboardEvent, 'key'>): boolean {
...
export const ShortcutRecordingDialog: React.FC<ShortcutRecordingDialogProps> = ({
  ...
  const handleRecordingEvent = (event: React.KeyboardEvent<HTMLDivElement>, phase: 'keydown' | 'keyup') => {
    ...
    if (phase === 'keydown' && isShortcutRecordingDismissEvent(event)) {
      close();
      return;
    }
