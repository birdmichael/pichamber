...
test('mobile comparisons drill into files, retry, resume, change source, and yield to external working diffs', async () => {
  ...
  try {
    ...
    expect(modeTrigger?.textContent).toBe('Git');
    ...
    expect(container.textContent).toContain('Failed to get git range diff');
    ...
    await click('[aria-label="Back"]');
    expect(container.querySelector('[aria-label="Select change mode"]')?.textContent).toBe('Git');
    ...
    directory = '/repo-two';
    await render();
    // Directory switch resets comparison navigation; wait out any ensureAll settle.
    await act(async () => { await Promise.resolve(); });
    const modeAfterSwitch = container.querySelector('[aria-label="Select change mode"]');
    if (!modeAfterSwitch) {
      // Loading/checking chrome uses an h2 title instead of the mode trigger.
      expect(container.querySelector('h2')?.textContent).toBe('Git');
    } else {
      expect(modeAfterSwitch.textContent).toBe('Git');
      expect(container.querySelector('h2')).toBeNull();
    }
