...
mock.module('@/stores/useGlobalSessionsStore', () => ({
  useGlobalSessionsStore: { getState: () => ({ upsertSession: () => undefined }) },
}));
...
const makeSession = (id: string, directory?: string, metadata?: Record<string, unknown>): Session => ({
  id,
  directory,
  title: 'btw: q',
...
beforeEach(() => {
  ...
  patchSessionMetadataImpl = (sessionId, _directory, updater) => {
    ...
    return Promise.resolve(makeSession(sessionId, '/project', result));
  };
});
...
describe('startBtwSession', () => {
  test('forks via the session host, marks the fork, links the parent, and routes the question to the fork', async () => {
    forkSessionImpl = (sessionId, messageId, directory) => {
    ...
    expect(isHiddenBtwSession(childStoreSessions[0])).toBe(true);
    expect(sentText).toBe('what is kafka');
...
describe('promoteBtwSession', () => {
  ...
  test('unlinks the parent, strips the marker so the fork is listable, and navigates to it', async () => {
    patchSessionMetadataImpl = (sessionId, _directory, updater) => {
    ...
    expect(isHiddenBtwSession(makeSession('fork-1', '/project', metadataPatches[1]?.result))).toBe(false);
