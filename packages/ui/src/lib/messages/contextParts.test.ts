...
describe('model-facing text', () => {
    ...
    test('terminal drafts fall back to language when terminalId is absent', () => {
        const payload = contextPayloadFromDraft(draft({
            source: 'terminal',
            fileLabel: 'Terminal 1',
            language: 'term-legacy',
            startLine: 1,
            endLine: 1,
            code: 'ok',
            text: '',
        }));
        expect(payload).toMatchObject({ kind: 'terminal', terminalId: 'term-legacy' });
    });
    ...
    test('preview-console keeps the pre-metadata wording', () => {
        expect(formatContextText(contextPayloadFromDraft(draft({
            source: 'preview-console',
            fileLabel: 'localhost:5173',
            language: 'log',
            code: 'GET /',
            text: 'why?',
        })))).toBe('Attached preview context from `localhost:5173`:\n```log\nGET /\n```\n\nwhy?');
    });
...
describe('round-trip through part metadata', () => {
    ...
    test('every draft-based kind survives create → read unchanged', () => {
        const payloads = [
            ...
            contextPayloadFromDraft(draft({ source: 'preview-console', language: 'log' })),
    ...
    test('legacy openchamberContext metadata still reads', () => {
        const payload = contextPayloadFromDraft(draft({ source: 'chat-quote', fileLabel: 'msg_1' }));
        expect(readContextPart({
            type: 'text',
            metadata: { openchamberContext: payload },
        })).toEqual(payload);
    });
...
describe('draft restore', () => {
    test('draft-based kinds survive payload → draft → payload', () => {
        const payloads = [
            contextPayloadFromDraft(draft()),
            contextPayloadFromDraft(draft({ source: 'file', side: undefined })),
            contextPayloadFromDraft(draft({ source: 'terminal', terminalId: 'term-1', language: '' })),
            contextPayloadFromDraft(draft({ source: 'preview-annotation' })),
            contextPayloadFromDraft(draft({ source: 'preview-console', language: 'log' })),
            contextPayloadFromDraft(draft({ source: 'pr-comment' })),
        ...
        for (const payload of payloads) {
            const restored = draftFromContextPayload(payload);
            expect(restored).not.toBeNull();
            expect(contextPayloadFromDraft({
                id: 'icd-restore',
                sessionKey: 'forked',
                createdAt: 2,
                ...restored!,
    ...
    test('linked issues and PRs have no draft form', () => {
        expect(draftFromContextPayload({ kind: 'github-issue', number: 3, title: 'Bug', url: 'https://x/issues/3' })).toBeNull();
        expect(draftFromContextPayload({ kind: 'github-pr', number: 7, title: 'PR', url: 'https://x/pr/7' })).toBeNull();
        expect(draftFromContextPayload({ kind: 'linear-issue', identifier: 'ENG-12', title: 'Login', url: 'https://linear.app/x/issue/ENG-12' })).toBeNull();
    });
});
