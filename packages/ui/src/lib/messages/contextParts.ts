...
export const CONTEXT_METADATA_KEY = 'pichamberContext';
const LEGACY_CONTEXT_METADATA_KEY = 'openchamberContext';
...
type PreviewConsoleContext = {
    kind: 'preview-console';
    fileLabel: string;
    language: string;
    output: string;
    text: string;
 ...
 * matches what Pichamber sent before parts carried metadata, so model
...
export function formatContextText(payload: ContextPartPayload): string {
    switch (payload.kind) {
        ...
        case 'preview-console':
            return `Attached preview context from \`${payload.fileLabel}\`:\n\`\`\`${payload.language}\n${payload.output}\n\`\`\`${payload.text ? `\n\n${payload.text}` : ''}`;
        case 'pr-comment':
...
export function contextPayloadFromDraft(draft: InlineCommentDraft): ContextPartPayload {
    switch (draft.source) {
        case 'terminal':
            return {
                kind: 'terminal',
                terminalId: draft.terminalId ?? draft.language,
...
const contextPayloadSchema = z.discriminatedUnion('kind', [
    ...
    z.object({
        kind: z.literal('preview-console'),
        fileLabel: z.string(),
        language: z.string(),
        output: z.string(),
        text: z.string(),
...
const metadataRecord = (part: ContextCarrierPart): Record<string, unknown> | undefined => {
...
export function readContextPart(part: ContextCarrierPart): ContextPartPayload | null {
    ...
    const parsed = contextPayloadSchema.safeParse(
        metadata?.[CONTEXT_METADATA_KEY] ?? metadata?.[LEGACY_CONTEXT_METADATA_KEY],
    );
...
/**
 * The composer draft a context payload came from, so forking a message can
 * put its attached context back on the chips instead of dropping it. Linked
 * issues/PRs have no draft form — they are owned by their own pickers — so
 * they map to null.
 */
