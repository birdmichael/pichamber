...
describe('shortcut schema', () => {
  ...
  test('keeps the mod+k leader for open/go actions', () => {
    ...
    expect(getShortcutAction('open_command_palette')?.defaultBinding).toBe('mod+p');
  });
  ...
  test('keeps Pichamber panel and Pi status actions', () => {
    expect(getShortcutAction('open_status')?.defaultBinding).toBe('mod+shift+o');
    expect(getShortcutAction('toggle_right_sidebar')?.defaultBinding).toBe('mod+b');
    expect(getShortcutAction('open_right_sidebar_git')?.defaultBinding).toBe('mod+shift+g');
