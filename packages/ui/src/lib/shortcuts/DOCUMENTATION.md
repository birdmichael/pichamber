...
The default layout follows three modes: single chords for everyday actions, the `mod+k` leader for open/go actions (`mod+k p`, `mod+k g`, `mod+k l`, `mod+k t`, `mod+k n`, `mod+k i`, `mod+k h`, `mod+k r`, `mod+k a`, `mod+k c`), and held digit prefixes — held `mod` + digit switches header session tabs, held `mod+alt` + digit switches context panel surfaces. `open_command_palette` stays `mod+p`; the leader must not open a second palette. Every schema action ships with a default binding; palette-only commands live outside the schema. Single-chord handlers still get the first chance at a leader's chord; returning `false` lets the dispatcher arm the sequence. Pi status (`open_status`) is an internal schema action. Pichamber-owned panel actions (`toggle_right_sidebar`, Git/Files surfaces, plan, services tab) stay in the schema with their existing defaults.
...
Local key handling remains appropriate for text editing, IME composition, menu and list navigation, dialog confirmation, terminal input, and other interactions that do not represent configurable application commands. The settings recorder treats Enter as a recordable key. Escape and X close the dialog; only Confirm applies a recording.
