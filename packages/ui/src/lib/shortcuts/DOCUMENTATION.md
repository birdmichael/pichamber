...
`switch_session_tab` (`mod` + digit) and `switch_context_surface` (`mod+alt` + digit) both keep the shortcut while the composer is focused — prefer tab/surface switching over inserting a digit. An unmodified sequence completion typed into a different editable target than the one that armed the leader is typing, not a shortcut.
