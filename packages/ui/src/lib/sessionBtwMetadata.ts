...
/** Temporary btw forks stay out of sidebar and session lists until Keep. */
export const isHiddenBtwSession = (session: Session | null | undefined): boolean =>
  isBtwSession(session);

/** Session lists omit hidden btw forks until Keep strips the marker. */
export const filterListedSessions = <T extends Session>(sessions: readonly T[]): T[] =>
  sessions.filter((session) => !isHiddenBtwSession(session));
