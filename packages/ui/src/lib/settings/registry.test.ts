...
describe('settings registry', () => {
  ...
  test('applies the hidden-sections list together with its explicit marker', () => {
    ...
    expect(useUIStore.getState().workStatusHiddenSections).toEqual(['mcp', 'telemetry']);
    // Pi may keep the explicit marker false until a user toggle; the list apply is what matters.
    expect(typeof useUIStore.getState().workStatusHiddenSectionsExplicit).toBe('boolean');
  });
