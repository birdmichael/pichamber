...
const isAmbiguousSendFailure = (error: unknown): boolean => {
...
/** Compatibility entry used by Header / session row "move to new worktree". */
export const startSessionTreeWorktreeMove = (input: {
  ...
  requestSessionTreeMove({
    ...
    messages: {
      ...
      sourceVerificationFailed: input.sourceVerificationFailed,
      applyChangesFailed: input.applyChangesFailed,
      changesMayBeInDestination: input.changesMayBeInDestination,
    },
  });
};
