...
describe('isAppLinkUrl', () => {
  test('accepts custom application schemes', () => {
    ...
    expect(isAppLinkUrl('spotify://track/abc')).toBe(true);
  });
  ...
  test('rejects OpenChamber, Pichamber, and Capacitor self-deep-links', () => {
    expect(isAppLinkUrl('openchamber://connect?host=x')).toBe(false);
    expect(isAppLinkUrl('openchamber-ui://app/index.html')).toBe(false);
    expect(isAppLinkUrl('pichamber://connect?host=x')).toBe(false);
    expect(isAppLinkUrl('pichamber-ui://app/index.html')).toBe(false);
