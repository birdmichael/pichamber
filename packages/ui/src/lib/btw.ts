 ...
 * as its window context. The fork is created through the existing session
 * host (`opencodeClient.forkSession` → Pi `host.forkSession` on the default
 * kernel), so the main chat's `currentSessionId` is never switched; the
 ...
 * already marked `kind: 'btw'` before this insert, so session lists hide it.
 */
