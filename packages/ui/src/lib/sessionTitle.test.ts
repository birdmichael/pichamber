...
import { collectSessionTitleTurns, formatSessionTitleContext, generatedSessionTitleSchema, generateSessionTitle, isPlaceholderSessionTitle, resolveSessionDisplayTitle, resolveSessionDisplayTitleFrom, shouldPersistSessionTitle } from './sessionTitle';
...
describe('formatMessageText for title context', () => {
  test('formats browser, PR, code and terminal contexts without mixing comments into quotes', () => {
    const contexts: ContextPartPayload[] = [
      { kind: 'browser-annotation', pageUrl: 'https://example.com', prompt: 'Page prompt', text: 'Look here' },
    ...
    expect(output).toContain('Browser annotation on https://example.com:');
    expect(output).toContain('> Page prompt');
    expect(output).toContain('**User comment:**\n\nLook here');
    expect(output).toContain('GitHub PR comment, #7 review:');
    expect(output).toContain('GitHub PR check, CI:');
