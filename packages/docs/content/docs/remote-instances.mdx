...
The desktop app can connect to Pichamber running on another machine over SSH — a work server, a cloud box, a homelab — and bring its UI to your screen as if it were local. Set this up at **Settings → Remote Instances**.
...
1. Open **Settings → Remote Instances** and choose **Add SSH connection**.
2. Pick a host from **From SSH config**, or switch to **Type it myself** and enter the SSH command plus a nickname.
3. Choose how Pichamber runs there:
   - **managed** — Pichamber installs and starts itself on the remote machine
   - **external** — connect to one that's already running
4. Ports, install method, passwords, and extra forwards sit under **Advanced**. The defaults work for most connections.
5. Connect.

Each connection shows **Connected**, **Connecting**, or **Needs attention**. When something fails, the row shows the failure text and a resolve button when the desktop app can fix it.

On a managed connection, Pichamber looks for a `pichamber` binary on the remote machine, including installs under the home directory. If none is there, you see that before connect starts — not a stack trace. Disconnecting without keeping the server running stops the remote Pichamber.

A managed remote can also listen on the remote machine's network (`0.0.0.0`) so other devices there can open it directly. That requires a UI password.
