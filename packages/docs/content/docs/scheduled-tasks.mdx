...
4. Set what it does: the prompt to send, and the provider, model, thinking level (when the model offers one), and agent to use. The prompt can be a slash command, like `/review`. The task list shows the model and thinking level you saved.
