...
2. Choose **Add SSH connection**, then pick a host under **From SSH config**.
3. If the host is a pattern, enter the real destination, such as `deploy@app.example.com`.
4. Save and connect.

Pichamber creates a remote instance using that SSH command. When the row shows **Connected**, the remote Pichamber UI opens in the desktop app.

## Add extra port forwards

Each remote instance has extra port forwards under **Advanced**. Use them when something on one side of the SSH connection needs to reach a port on the other side.
