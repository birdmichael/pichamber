---
title: Tüneller
description: Uzak ve mobil erişim için Pichamber'ı güvenli biçimde açığa çıkarın.
---

# Tüneller

Bir tünel, Pichamber'ınıza giden genel bir bağlantıdır. Böylece başka bir ağdaki normal bir tarayıcıdan erişebilirsiniz. Çalışan bir örnek için bir tane oluşturmak üzere `openchamber tunnel` kullanın.
...
Pichamber, tünel sağlayıcısının CLI'sını sizin makinenizde başlatır. Önce kullanmak istediğiniz sağlayıcıyı kurun:
...
Tünel ayaktayken `status` bir genel URL gösterir. Onu açın ya da QR kodunu tarayın, Pichamber'a her yerden ulaşın.

Varsayılan olarak Pichamber, etkileşimli TTY oturumlarında bir QR kodu basar. QR çıktısını zorlamak için `--qr`, kapatmak için `--no-qr` kullanın.
...
- Pichamber örneği başına bir aktif tünel, yani port
...
- [PWA & Mobile](/mobile/) — telefonunuzdan Pichamber'a ulaşın
