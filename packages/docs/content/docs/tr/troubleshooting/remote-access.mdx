...
description: Tünelleri, uzak instance'ları ve Pichamber'a başka bir cihazdan erişmeyi düzeltin.
---

# Uzaktan Erişim

Pichamber'a telefonunuzdan ya da başka bir makineden ulaşamıyorsanız, çözüm bağlantı biçiminize göre değişir.
...
- **Anywhere** eşleştirmesi için sunucudaki **Settings → Remote Instances → Pichamber Relay** ayarını kontrol edin. **Connected** yazmalı. Değilse devre dışı bırakıp yeniden etkinleştirin
...
Bir [remote instance](/remote-instances/) takılırsa, Pichamber hangi adımın başarısız olduğunu söyler:

- **auth** — SSH ya da UI parolanız reddedildi. Tekrar girin
- **install / start** — Pichamber uzak makinede sunucuyu kuramadı ya da başlatamadı. O makinenin gereksinimlerini kontrol edin
- **forwarding** — bağlantı kuruludur ama port size ulaşmıyordur. Farklı bir local port deneyin

## Kendi sunucunuzun arkasında

Pichamber'ı bir reverse proxy arkasına koyduysanız ve garip açılıyorsa ya da bağlanmıyorsa, [Reverse Proxy](/reverse-proxy/) bölümüne bakın.
