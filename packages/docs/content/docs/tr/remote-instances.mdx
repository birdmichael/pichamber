---
title: Uzak örnekler
description: Masaüstü uygulamasını SSH üzerinden başka bir makinedeki Pichamber'a bağlayın.
---

# Uzak örnekler

Masaüstü uygulaması, başka bir makinede çalışan Pichamber'a SSH üzerinden bağlanabilir. Bu bir iş sunucusu, bulut kutusu ya da ev laboratuvarı olabilir. Arayüzünü yerelmiş gibi ekranınıza getirir. Bunu **Ayarlar → Uzak örnekler** bölümünden kurun.
...
3. Pichamber'ın orada nasıl çalışacağını seçin:
   - **managed** — Pichamber kendini uzak makineye kurar ve başlatır
   - **external** — zaten çalışan bir örneğe bağlanır
4. Bağlanın.

Pichamber adımları tek tek ilerletir. Bağlantıyı kontrol eder, uzak ortamı hazırlar, sunucuyu başlatır ve portu yönlendirir. Her aşamada nerede olduğunu gösterir. **ready** noktasına geldiğinde uzak arayüz yerelde açılır.

## Kimlik bilgileri

SSH ve arayüz parolalarını kaydetmeyi ya da her seferinde girmeyi siz seçersiniz. Bağlantı koparsa Pichamber hangi adımın başarısız olduğunu bildirir, böylece düzeltebilirsiniz. Ayrıntı için [Uzak erişim](/troubleshooting/remote-access/) sayfasına bakın.

## Bağlantı bağlantıları

Uzak makinede Pichamber zaten çalışıyorsa, masaüstü uygulamasını bağlamanın en kolay yolu eşleştirme bağlantısıdır. Uzak sunucunun arayüzünde **Ayarlar → Uzak örnekler → Bu sunucuya bağlan → Bir cihaz ekle** yolunu açın, bir bağlantı oluşturun ve masaüstünüzde **Ayarlar → Uzak örnekler → Diğer Pichamber sunucuları → Bağlantıyı içe aktar** bölümünden içe alın. Tam akış için [Cihaz bağlama](/connect-devices/) sayfasına bakın.
