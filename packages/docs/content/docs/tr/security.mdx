...
Pichamber makinenize ve kodunuza erişim verir. Bu yüzden sizden başkası ulaşmadan önce kilitleyin. Bu sayfa arayüz parolasını, passkey'leri ve Pichamber'ı bir ağa açmadan önce bilmeniz gerekenleri anlatır.

## Arayüz parolası ayarlama

Pichamber'ı bir parola ile başlatın, tarayıcı arayüzü bunu ister:

```bash
openchamber --ui-password be-creative-here
```

Parolayı komut satırına yazmak yerine `OPENCHAMBER_UI_PASSWORD` ortam değişkeniyle de ayarlayabilirsiniz. Oturum açtıktan sonra Pichamber bir süre cihazı hatırlar, böylece her seferinde sorulmaz.
...
Parola ayarlandıktan sonra daha hızlı giriş için passkey ekleyebilirsiniz. Bunlar Face ID, Touch ID ya da bir güvenlik anahtarı olabilir. **Ayarlar → Pichamber → Passkeys** bölümünden ekleyin.
...
- Varsayılan olarak Pichamber yalnızca kendi makinenizde dinler (`127.0.0.1`). Daha geniş dinlemek için bilinçli bir değişiklik gerekir ve önce parola ayarlamalısınız.
...
- Pichamber'ı kendi HTTPS sunucunuzun arkasına koyuyorsanız [Reverse Proxy](/reverse-proxy/) sayfasına bakın.
...
- [Reverse Proxy](/reverse-proxy/) — Pichamber'ı kendi sunucunuzun arkasında çalıştırma
