...
description: Pichamber'ın otomatik akışlarının arkasındaki yerleşik istemleri özelleştirin.
---

# Sihirli İstemler

Pichamber bir şeyi otomatik yaptığında arka planda yerleşik istemler kullanır. Commit mesajı yazmak, PR taslağı hazırlamak, issue incelemek, çakışmayı çözmek, bir oturumu özetlemek gibi. Magic Prompts, bu istemleri okuduğunuz ve yeniden yazdığınız yerdir. **Settings → Magic Prompts** bölümünden açın.
...
Bazı istemlerin görünen bir kısmı vardır. Bu, sizin göreceğiniz mesajdır. Bir de agent için gizli yönerge kısmı vardır. İstemler `{{placeholders}}` içerebilir. Pichamber bunları doldurur. Mesela diff ya da issue başlığı gibi. Bunları olduğu gibi bırakın.
