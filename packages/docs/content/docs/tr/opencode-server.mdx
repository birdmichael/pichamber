...
description: Pichamber'ı yerel veya uzak bir OpenCode sunucusuna bağlayın.
---

# OpenCode Sunucusu

Pichamber, bir OpenCode sunucusunun üzerinde çalışır. Varsayılan olarak sizin için bir sunucu başlatır; bu nedenle bir şey yapmanız gerekmez. Bu sayfaya yalnızca Pichamber'ı zaten çalıştırdığınız bir sunucuya yönlendirmek veya onun başlattığı sunucuyu yönetmek istiyorsanız ihtiyacınız olur.

## Pichamber sunucuyu nasıl bulur

Pichamber başladığında sunucuyu şu sırayla arar:
...
Pichamber'ı başlatmadan önce şunları ayarlayın:
...
- `OPENCODE_SKIP_START=true`, Pichamber'a kendi sunucusunu başlatmamasını söyler.

Yalnızca bağlantı noktasını değiştirmek istiyorsanız `OPENCODE_PORT` öğesini `OPENCODE_HOST` yerine ayarlayın.

`OPENCODE_HOST` bağlantı noktasını içermiyorsa veya bir yol içeriyorsa Pichamber bunu yok sayar ve kendi sunucusunu başlatmaya döner. Beklediğiniz bağlantı kurulmadıysa başlangıç günlüklerindeki `[config]` uyarısına bakın.
...
## Pichamber'ı oturum açınca başlatın

Yerel bir kullanıcı hizmeti kurmak için `startup enable` kullanın. Pichamber macOS'ta `launchd`, Linux'ta `systemd --user`, Windows'ta Task Scheduler kullanır.
...
Başka bir Pichamber uygulaması için bağlantı bağlantısı oluşturmak üzere şunu kullanın:
...
Sunucu başlarken veya yeniden başlarken Pichamber "OpenCode yeniden başlatılıyor" durumunu gösterir ve hazır olana kadar istekleri duraklatır. Bu, uygulama açıldıktan veya yeniden başlatıldıktan hemen sonra normaldir. Durum hiç kaybolmazsa [OpenCode bağlantısı](/troubleshooting/opencode-connection/) sayfasına bakın.
