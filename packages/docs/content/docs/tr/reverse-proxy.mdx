---
title: Reverse Proxy
description: Pichamber'ı Nginx, Nginx Proxy Manager veya başka bir reverse proxy arkasında doğru şekilde ayarlayın.
---

# Reverse Proxy

Pichamber'ı Nginx, Nginx Proxy Manager, Caddy, Cloudflare ya da başka bir reverse proxy arkasında çalıştırıyorsanız bu sayfayı kullanın.

## Proxy'lemeden önce

1. Önce Pichamber'ın doğrudan çalıştığını doğrulayın.
...
- Pichamber yanıtları zaten sıkıştırıyorsa proxy'de gzip'i kapatın.
...
- Pichamber LAN üzerinden doğrudan erişilebilir
...
- Pichamber HTTP yanıtlarını gzip ile sıkıştırır, eşik 1 KB'dir.
...
SSE akış rotaları Pichamber tarafından sıkıştırmanın dışında tutulur, ancak CDN bunları yine de buffer'layabilir. SSE yollarında buffer'lamayı nasıl kapatacağınızı CDN belgelerinden kontrol edin.
