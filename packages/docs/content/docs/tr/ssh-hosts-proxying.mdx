...
Pichamber, yerel SSH yapılandırmanızdaki host'ları okuyabilir. Bu, `ssh work-server` gibi komutların kullandığı yerdir.
...
Pichamber o SSH komutunu kullanarak bir uzak örnek oluşturur. **ready** noktasına geldiğinde uzak Pichamber arayüzü masaüstü uygulamasında açılır.
...
Pichamber üç yönlendirme tipini destekler:
...
Bilgisayarınızdaki diğer araçların uzak makine üzerinden gezmesini istiyorsanız **Dynamic (-D)** seçin. Pichamber, o SSH bağlantısı için yerel bir SOCKS proxy portu açar.
...
- [Uzak örnekler](/remote-instances/) — masaüstü uygulamasını başka bir makinedeki Pichamber'a bağlayın
