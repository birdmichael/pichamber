...
- confirm the server is running with `pichamber status`
...
- for **Anywhere** pairing, check **Settings → Remote Instances → Pichamber Relay** on the server: it should say **Connected**; if not, disable and re-enable it
...
When a [remote instance](/remote-instances/) stalls, the row shows **Needs attention** with the failure text and a resolve button when the desktop app can fix it:

- **auth** — your SSH or UI password was rejected; re-enter it
- **install / start** — Pichamber couldn't set up or start the server on the remote machine. A missing remote Pichamber kernel is reported before connect. Install it there and try again
