...
async function scheduleCommand(options = {}, action = 'help') {
  if (action === 'help') {
    process.stdout.write(`OpenChamber Schedule Commands\n\nUSAGE:\n  pichamber schedule status [OPTIONS]\n  pichamber schedule list (--project <projectId> | --dir <path>) [OPTIONS]\n  pichamber schedule create (--project <projectId> | --dir <path>) --name <name> --prompt <prompt> --model <provider/model> (--daily <HH:mm> | --weekly <0,1,2> --time <HH:mm> | --once <YYYY-MM-DD> --time <HH:mm> | --cron <expr>) [OPTIONS]\n  pichamber schedule run (--project <projectId> | --dir <path>) --task <taskId> [OPTIONS]\n  pichamber schedule delete (--project <projectId> | --dir <path>) --task <taskId> [OPTIONS]\n  pichamber schedule enable (--project <projectId> | --dir <path>) --task <taskId> [OPTIONS]\n  pichamber schedule disable (--project <projectId> | --dir <path>) --task <taskId> [OPTIONS]\n\nOPTIONS:\n  --project <projectId>   Project id from pichamber projects\n  --dir <path>            Resolve project by directory\n  -p, --port <port>       OpenChamber server port\n  --timezone <zone>       IANA timezone for created tasks\n  --agent <id>            Agent to use when running task\n  --variant <id>          Model variant to use when running task\n  --goal                  Continue the scheduled session toward a goal\n  --goal-token-budget <n> Goal token budget (1000-100000000; requires --goal)\n  --disabled              Create task disabled\n  --json                  Output machine-readable JSON\n  -q, --quiet             Print concise output\n`);
    return;
  }
