...
- `getRangeFiles(directory, { base, head })`: Get list of changed files between two refs. Uses `git diff --name-status -z -C` so rename/copy entries report the destination path. Exposed as `GET /api/git/range-files`.
- `getBranchBase(directory, branch)`: Resolve the branch this branch was created from using its local reflog (`branch: Created from <ref>`). Returns `{ base: null }` when git has no authoritative record (clone, detached start, expired reflog, raw commit) **or** when the recorded source is this branch itself or its own remote-tracking copy (`origin/<branch>`). A fetched feature worktree typically records `Created from origin/<same-branch>`; that is not a parent and must not produce a 0-file self-diff. A differently named parent (`origin/main`) is kept even when HEAD still matches that commit. Never guesses `main`/`master`. Exposed as `GET /api/git/branch-base`. Git commands run in the requested workspace directory.
...
- `GET /api/git/range-diff`, `GET /api/git/range-files`, and `GET /api/git/branch-base` are served by the Pichamber web server, so they are available to web, desktop, and mobile clients. The shared `GitAPI.getGitRangeDiff`, `getGitRangeFiles`, and `getBranchBase` methods are therefore optional: web supplies the HTTP implementation, and VS Code does not implement them because the leftover extension host serves Git through its own bridge rather than these routes. Features built on range diffs (the context-panel Branch scope and the AI diff walkthrough) are not offered in VS Code. All three operations stay inside the requested workspace directory.
