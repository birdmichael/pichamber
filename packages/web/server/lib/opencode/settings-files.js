...
export const buildPreferencesFields = (previousFields, document, now, { surface = null, changedKeys = null } = {}) => {
  ...
  // A flat merged document may omit per-surface-only keys (no base value). Keep
  // those entries so a migration rewrite does not wipe Desktop overlays.
  for (const [key, previous] of Object.entries(previousFields)) {
    if (fields[key] || !previous || !isProfileSettingsKey(key)) continue;
    if (previous.surfaces || 'value' in previous) {
      fields[key] = previous;
    }
  }
  return fields;
