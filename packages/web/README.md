...
> **Prerequisites:** Node.js 22+ and Bun. The default kernel is in-process Pi (`OPENCHAMBER_KERNEL` defaults to `pi`). OpenCode CLI is only needed if you set `OPENCHAMBER_KERNEL=opencode`.
...
pichamber connect-url --port 3000  # Add this server to Pichamber Desktop
...
When Pichamber launches with the leftover OpenCode process kernel, it also registers a native
...
- One active tunnel per running Pichamber instance (port).
...
### Connect other Pichamber apps

Use `connect-url` when a web/API server should be added to Pichamber Desktop or another Pichamber app. If no server is running on the selected port, Pichamber starts one first.
...
| `OPENCHAMBER_HOST` | Bind hostname for the Pichamber web server (default: `127.0.0.1`; use `0.0.0.0` for LAN/remote access — trusted networks only) |
