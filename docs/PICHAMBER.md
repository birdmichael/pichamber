...
- VS Code, Windows, and Linux desktop packaging were not the product target for this kernel swap. Hosted mobile / Capacitor is a touch client of the same Pi host: Settings, View Plan, and session chrome (Scheduled Tasks, Multi-run, Archive+restore, Pi Refresh) now follow the same Pi APIs as Desktop; later slices still cover Work Status and Phase 2 workspace surfaces. Desktop-native privileges (SSH, External Tunnel host, Electron browser session) stay unsupported.
