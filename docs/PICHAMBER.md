...
- Session export/import: `GET|POST /api/session/:id/export?format=jsonl|html` and `POST /api/session/import`. JSONL export writes Pi-native `text` / `thinking` / `toolCall` / `toolResult` / `image` so import reconstructs the same facade parts, and keeps assistant `provider` / `model` / `usage` when they already exist. HTML export writes a self-contained offline file: Markdown text, thinking as its own block, tool calls with input/output, and embedded `data:` images. Remote http(s) image URLs are omitted or labeled. It does not create a public share URL. JSONL stays the round-trip / re-import format.
