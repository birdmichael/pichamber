...
- Session plan chrome when Feature Plugins `plan` is installed and enabled: `GET|POST /api/pi/session/:id/plan` from live `plan-mode-state` (not TUI, not `.opencode/plans`). Composer **Agent | Plan** shows on an idle empty session (status defaults to `off`) and uses `/plan start` (toast only; not a question-card probe). Bare `/plan` from the slash menu still opens the plugin launch card and must appear as live `/plan`, not only `/plan-feature`. View Plan rail while Plan is `active`/`ready`/`saved`/`implementing` (empty “no plan yet” when the model never wrote markdown), and Build (`/plan implement` in this session) only when markdown exists. Leave a ready plan saves; leave Plan with no document uses `/plan exit`. Discard is confirm + `/plan exit` from the panel only. A saved plan resumes without `/plan start`.
