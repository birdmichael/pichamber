...
- Session CRUD, `prompt_async` / `prompt`, abort, messages, status. New sessions use Pi's persisted UUID under `~/.pi/agent/sessions`; `GET /api/session/:id` and `.../message` open a listed disk session after restart (same id as `pi` CLI for that cwd). Hydrate maps Pi `toolCall` + `toolResult` onto assistant tool parts so a reload does not turn Read/Skill/Bash output into user markdown. Session Goal metadata and Archive (`time.archived`) are written through the same `PATCH /api/session/:id` path and stored as a Pi `custom` entry (`pichamber.metadata`) on that jsonl so the goal and archive flag survive reload. Restore writes `time.archived = 0`. A live in-memory archive/title mutation wins over a disk row that is missing the flag. When the Subagents feature-plugin slot is on, `GET /api/session/:id/subagent-runs` lists adapter children and `GET /api/session/:id/children` returns attached child session infos (not leftover `parentID` clones). A child with a session file is a real facade session (`prompt` / steer stay on that child).
