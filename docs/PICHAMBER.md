...
- VS Code, Windows, and Linux desktop packaging were not the product target for this kernel swap. Hosted mobile / Capacitor is a touch client of the same Pi host: Settings and View Plan now follow Desktop Feature Plugin gates; later slices still cover session chrome, Work Status, and Phase 2 workspace surfaces. Desktop-native privileges (SSH, External Tunnel host, Electron browser session) stay unsupported.
