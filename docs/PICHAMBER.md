...
- Event mapping: `text_delta` → `message.part.delta` field `text`; `thinking_delta` → reasoning; `tool_execution_*` → tool parts; `agent_start` → busy; `agent_settled` → idle; successful `compaction_end` → `session.compact` end plus `session.compacted` (pin inject). Abort or failed compact does not emit `session.compacted`.
