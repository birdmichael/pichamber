...
- Providers from `ModelRuntime.getAvailable()` (`GET /api/config/providers` is the live connected list and must not keep a model-less xAI stub). `GET /api/provider` is `{ all, default, connected }`: `all` merges the built-in xAI catalog so Add Provider works before login; Settings Add opens Other / Custom when every catalog id is already connected. Mock kernel still returns only `pi-mock`
