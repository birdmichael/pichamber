...
- Chat: a parent user bubble beside Work Status and a child tab wraps in the column instead of collapsing to a one-line ellipsis. Composer model labels ellipsize (`grok-…`); the entire Agent chip hides at the live ~328px / 20.5rem squeeze instead of clipping mid-glyph (`A` plus a sliver, or `Agen`). The parent composer footer is the `@container model-controls` query box so that hide uses the ~328px column, not the window.
