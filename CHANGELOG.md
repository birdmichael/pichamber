...
- **Chat:** on Pi, when the Goal plugin is installed and enabled, the composer shows a Goal button. Click asks for an objective, then starts `/goal <objective>` on the current session. An empty objective cannot submit. If the command is missing, the button stays and shows an error instead of sending a chat message. OpenChamber Session Goal stays hidden on Pi.
