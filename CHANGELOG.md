...
- Chat: on Pi, a refresh button next to the session title reloads skills, prompts, and extensions in place — the current session stays open. Slash and Settings lists update afterward, and a short toast confirms the reload. It is disabled while the session is composing, streaming, or compacting.
