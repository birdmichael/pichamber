...
- Chat: a parent user bubble beside Work Status and a child tab wraps in the column instead of collapsing to a one-line ellipsis. Composer model labels ellipsize (`grok-…`); the entire Agent chip hides through the child footer band (~27rem / 432px) instead of clipping mid-glyph (`Ag(`). The thinking label hides at the same 22rem squeeze so the icon cannot paint over High/Medium. The parent composer footer is the `@container model-controls` query box. The child panel X dismisses session-scoped child chats, not only the project directory key.
