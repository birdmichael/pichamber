...
- **Chat:** on Pi, Work Status Subagents and the dedicated transcript card appear only after the Subagents feature plugin is installed and enabled. Click a live child or the card to open that child and keep chatting there; the parent session stays unchanged. Phone and VS Code open the child in place. A run without a session id shows status only, and installing the plugin does not start a reviewer.
