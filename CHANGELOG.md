...
- Chat: a waiting Input needed card is one live card, not a greyed transcript copy plus a dock duplicate. Enter while that card is open says to submit or dismiss it, and Submit stays disabled until you pick an option. The busy line stays visible and says “asking a question.” Typing `/plan` on a new chat opens the Plan card instead of a crash toast or a leftover `/plan` bubble, and a late Plan menu does not stack on an earlier confirm.
