...
- Settings: custom GPT-5.5 / GPT-5.6 / GPT-6 models (for example `gpt-6-astra`) keep thinking instead of being treated as off-only after Fetch models. Saving a provider no longer drops a stored thinking map.
...
- Chat: a Plan-mode message that starts with `Plan ` (no slash) keeps that word in the bubble and session title. Only `/plan` slash commands are treated as commands.
...
- Files: right-clicking empty space in the file tree shows New File / New Folder, not the desktop Inspect Element menu.
...
- Mobile: Work Status includes pinned messages, context sources, and the section equalizer. Tapping a row closes the sheet and opens the matching workspace or review surface instead of writing to a Desktop panel that is not mounted.
- Chat: opening a Subagent child tab no longer squeezes the parent transcript under Work Status. The parent column keeps at least ~320px; the child pane and Work Status shrink instead. Work Status header uses `1 queued · 3 done`.
- Chat: a parent user bubble beside Work Status and a child tab wraps in the column instead of collapsing to a one-line ellipsis. Composer model labels ellipsize (`grok-…`); the entire Agent chip hides through a ~500px child footer (~36rem / 576px) instead of clipping mid-glyph (`Ager` / `Ag(`). The thinking chip (not just the label) hides at the 22rem squeeze, and the parent footer chip row keeps a real gap so the icon cannot sit on the preceding toolbar icon. Parent and child/embedded composer footers (and the chip row) are the `@container model-controls` query box. Parent ChatInput measures `[data-parent-chat-column]` and omits the Agent slot from the DOM below 576px (~328px squeeze); CSS hide remains backup when the footer or chip row is below 576px or the slot overflow-clips, so a parent main-window squeeze cannot paint `Ag` and a child iframe cannot keep querying a wide parent/window. The child panel X dismisses session-scoped child chats, not only the project directory key.
- Desktop: a background or hidden window no longer keeps painting at full rate. When the menu bar is on, counts still update from session events, and idle 5s/45s refresh sleeps until a session is working.
- Chat: on Pi, `/plan` `/run` `/goal` still run as commands when Feature Plugins have not finished loading. A pasted `/plan` with extra whitespace is the same command. Typing `/plan` on a new chat opens the Plan card instead of a leftover `/plan` bubble. Opening New session only drops a leftover `/`, so a real untitled draft stays. A waiting Plan card uses the short heading and scrolls into view.
- **Chat:** sending the first message in a new session shows your bubble immediately. The session can keep starting in the background. Picking Plan on a new draft toasts and shows `Plan starts when you send.` The chip stays on Plan after that first send. A finished assistant duration no longer keeps growing. The header title reload glyph no longer stays after the turn is idle. Leftover Recap and suggestion no longer appear after a normal turn.
...
- Chat: Work Status names queued and stopped Subagent rows instead of treating them as working or failed, and a child waiting on permission, a question, or a supervisor interview shows that state. A fan-out workflow lists each child. Switching conversations clears the previous chat’s fleet. A Subagent tab in the right panel stays with its parent chat, not the project. Opening a child restores that child’s model and thinking. After restart, a parent that actually launched a child can expand even when the child transcript is long or has no status file.
...
- Settings: External Tunnel Connect-link TTL options are now 3m, 30m, 2h, 8h, 24h.
- Settings: the Sessions model-picker capability popover stays above the settings list instead of clipping under it.
- Settings: Reload themes shows a success or error toast.
- Settings/Desktop: launch-at-login, tray, and keep-awake sit under Desktop app, separate from Network Access. Save and restart is outline and disabled until something changed.
- Settings: the LAN checkbox lines up with its label, and New MCP Server Name lines up with its input.
- Settings: Skills + opens the new-skill form on the first click. Session work status counts only injected skills, and Untrusted copy says those skills stay listed but are not injected.
- Settings: Providers Connect keeps Other/Custom visible when the catalog is empty, and says so instead of a search that hides it.
- Desktop: on unsigned or ad-hoc Mac builds, Restart to Update no longer pretends to succeed. The app opens the GitHub release so you can replace Pichamber.app from the .dmg.
- Desktop: dismissing the Pi update toast — Dismiss or OK — keeps that version hidden until a newer Pi version is available.
- Chat: after the multi-run model picker is closed, Esc now closes the New multi-run form, same as Cancel.
...
- Chat: the same model now shows one context size in Recent, the fork-session picker, and the multi-run picker — the live Pi catalog window, then that model's exact catalog or published window, not another provider's leftover number.
...
- Settings: adding or editing a custom provider now lets you pick OpenAI Chat Completions, OpenAI Responses, or Anthropic Messages instead of locking the form to OpenAI-compatible.
...
- Desktop: the right-hand rail shows Walkthrough at 768px and up, without opening Diff first. VS Code and narrower windows still hide it.
...
- Settings/Sessions: on Pi, leftover OpenCode agent dropdowns no longer appear in Commands Override Agent, Scheduled Tasks, Todo send, Fork, Review, or Multi-run. Those flows stay on the built-in Pi agent. Model pickers stay. The OpenCode kernel still has the agent list.
- Docs: the Providers page no longer tells you to open Settings → Agents and create personalities on Pi. It points to Feature Plugins (Plan / Subagents) and Session Defaults. The OpenCode kernel path stays documented.
...
- Mobile: on Pi, the workspace MCP tab now appears only when Feature Plugins MCP is installed and enabled, matching Settings. Add Server no longer opens an unavailable Settings page when the slot is off.
...
- **Sessions:** on Pi, archiving a chat now keeps it off the daily session-list scan. Startup and sidebar refresh stay proportional to open chats; restored chats return to that list, and Archive still shows archived ones.
- **Sessions:** on Pi, startup, sidebar Refresh, and the tray no longer load archived chats. Those appear when you open Archive, or the archived list in VS Code. OpenCode still loads both.
- Desktop: Archive's directory list no longer sits under the session sidebar, so "All directories" stays fully visible with the Git panel open or closed.
...
- Chat: on Pi, reopening or refreshing a finished session still shows copy, save as image, and pin on the assistant reply.
...
- **Settings/Providers:** custom models can now set an input context window. Fetch keeps a provider-reported size when `/models` includes one; typing a known id prefills that model’s published window (`gpt-4o` 128k, `gpt-4.1` 1M, current Claude / Grok / Gemini / DeepSeek ids). Leaving Context empty on a known id now stores that published window (Grok 4.6 is 500k) instead of letting the session fall back to 128k. A number you type is kept. A family guess is labeled and only used when both miss. Usage and the context panel use the stored window instead of the generic 200k fallback.
...
- **Chat:** on Pi, when the Plan plugin is installed and enabled, the composer shows Agent | Plan. Starting Plan turns Plan on with a toast; typing `/plan` still opens the in-chat launch menu. Leaving a ready plan saves it, Build runs `/plan implement` in this session with the current (or picked) model, and View Plan appears on the right rail while Plan is on (including an empty “no plan yet” state). Discard is a confirmed action on that panel, not from the footer.
- **Chat:** on Pi, Work Status Subagents and the dedicated transcript card appear only after the Subagents feature plugin is installed and enabled. Click a live child or the card to open that child and keep chatting there; the parent session stays unchanged. Phone and VS Code open the child in place. Leftover adapter rows without a session id are not listed; a run still starting can show status only. Installing the plugin does not start a reviewer.
- **Settings/MCP:** on Pi, Settings → MCP and the Work Status MCP section appear only after you install and enable the MCP adapter on Feature Plugins. Leftover `mcp.json` files do not show those surfaces. When the slot is on, adding or editing a server writes the adapter files (`~/.config/mcp/mcp.json` for you, `.mcp.json` in the project), turning a server off only writes `disabled` in `.pi/mcp.json`, and the current session reloads so tools update. Cached or not-yet-connected servers stay listed as normal.
- **Chat:** on Pi, when the Goal plugin is installed and enabled, the composer shows a Goal button. Click asks for an objective, then starts `/goal <objective>` on the current session. An empty objective cannot submit. Start failures stay visible in the dialog. If the command is missing, the button stays and shows an error instead of sending a chat message. OpenChamber Session Goal stays hidden on Pi.
- **Settings:** on Pi, Feature Plugins sits next to Extensions as a 2×2 card grid for Goal, Plan, MCP, and Subagents. Each card shows the default package, install state, enable, and Install / Uninstall. There are no source or start-command fields. Nothing is installed just by opening the page. After install or uninstall, idle sessions reload so commands and extensions refresh.
...
- Chat: on Pi, the composer no longer shows an agent chip that only says Pi. Model and thinking chips stay. OpenCode build, plan, and custom agents are unchanged.
- Chat: the thinking chip no longer flashes High before the saved default loads.
