...
- **Chat:** sending the first message in a new session shows your bubble immediately. The session can keep starting in the background. A Plan chip on that draft stays on Plan after send. A finished assistant duration no longer keeps growing. The header no longer stays busy after the turn is idle. Leftover Recap and suggestion no longer appear after a normal turn.
