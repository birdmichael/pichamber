...
- Chat: a parent user bubble beside Work Status and a child tab wraps in the column instead of collapsing to a one-line ellipsis. Composer model labels ellipsize (`grok-…`); the entire Agent chip hides through a ~500px child footer (~36rem / 576px) instead of clipping mid-glyph (`Ager` / `Ag(`). The thinking chip (not just the label) hides at the 22rem squeeze, and the parent footer chip row keeps a real gap so the icon cannot sit on the preceding toolbar icon. The parent composer footer is the `@container model-controls` query box. The child panel X dismisses session-scoped child chats, not only the project directory key.
